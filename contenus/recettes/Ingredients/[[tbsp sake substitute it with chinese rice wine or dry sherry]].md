---
type: ingredient
nom: '[[tbsp sake ((substitute it with chinese rice wine or dry sherry))]]'
categorie: ''
recettes:
- Tan Tan Ramen (Tantanmen) 坦々麺
tags:
- ingredient
---

# 🥕 [[tbsp sake ((substitute it with chinese rice wine or dry sherry))]]

## 📋 Informations

- **Catégorie**: 
- **Saison**: 
- **Allergènes**: 

## 🍽️ Utilisé dans les recettes

```dataview
TABLE WITHOUT ID
  file.link as "Recette",
  temps_preparation as "Préparation (min)",
  temps_cuisson as "Cuisson (min)",
  type_cuisine as "Cuisine"
FROM "contenus/recettes/Fiches"
WHERE contains(ingredients, "[[tbsp sake ((substitute it with chinese rice wine or dry sherry))]]")
SORT file.name ASC
```

## 💡 Notes


## 🔗 Liens
