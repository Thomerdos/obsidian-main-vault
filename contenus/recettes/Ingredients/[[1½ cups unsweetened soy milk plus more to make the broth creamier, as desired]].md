---
type: ingredient
nom: '[[1½ cups unsweetened soy milk ((plus more to make the broth creamier, as desired))]]'
categorie: ''
recettes:
- Tan Tan Ramen (Tantanmen) 坦々麺
tags:
- ingredient
---

# 🥕 [[1½ cups unsweetened soy milk ((plus more to make the broth creamier, as desired))]]

## 📋 Informations

- **Catégorie**: 
- **Saison**: 
- **Allergènes**: 

## 🍽️ Utilisé dans les recettes

```dataview
TABLE WITHOUT ID
  file.link as "Recette",
  temps_preparation as "Préparation (min)",
  temps_cuisson as "Cuisson (min)",
  type_cuisine as "Cuisine"
FROM "contenus/recettes/Fiches"
WHERE contains(ingredients, "[[1½ cups unsweetened soy milk ((plus more to make the broth creamier, as desired))]]")
SORT file.name ASC
```

## 💡 Notes


## 🔗 Liens
