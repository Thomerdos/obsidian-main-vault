---
type: ingredient
nom: '[[cup mini cucumber or regular cucumber, halved lengthwise and sliced on a bias]]'
categorie: ''
recettes:
- Sweet and Sour Stir Fry ผัดเปรี้ยวหวาน Recipe & Video Tutorial
tags:
- ingredient
---

# 🥕 [[cup mini cucumber or regular cucumber, halved lengthwise and sliced on a bias]]

## 📋 Informations

- **Catégorie**: 
- **Saison**: 
- **Allergènes**: 

## 🍽️ Utilisé dans les recettes

```dataview
TABLE WITHOUT ID
  file.link as "Recette",
  temps_preparation as "Préparation (min)",
  temps_cuisson as "Cuisson (min)",
  type_cuisine as "Cuisine"
FROM "contenus/recettes/Fiches"
WHERE contains(ingredients, "[[cup mini cucumber or regular cucumber, halved lengthwise and sliced on a bias]]")
SORT file.name ASC
```

## 💡 Notes


## 🔗 Liens
