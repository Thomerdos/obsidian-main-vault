---
type: ingredient
nom: '[[à café de vinaigre de vin blanc ou rouge selon la coloration que vous voulez
  donner]]'
categorie: ''
recettes:
- Navets glacés au miel
tags:
- ingredient
---

# 🥕 [[à café de vinaigre de vin blanc ou rouge selon la coloration que vous voulez donner]]

## 📋 Informations

- **Catégorie**: 
- **Saison**: 
- **Allergènes**: 

## 🍽️ Utilisé dans les recettes

```dataview
TABLE WITHOUT ID
  file.link as "Recette",
  temps_preparation as "Préparation (min)",
  temps_cuisson as "Cuisson (min)",
  type_cuisine as "Cuisine"
FROM "contenus/recettes/Fiches"
WHERE contains(ingredients, "[[à café de vinaigre de vin blanc ou rouge selon la coloration que vous voulez donner]]")
SORT file.name ASC
```

## 💡 Notes


## 🔗 Liens
