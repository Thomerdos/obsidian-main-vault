---
type: ingredient
nom: '[[katsuobushi (copeaux de bonite séchée)]]'
categorie: ''
recettes:
- Recette Japonaise - Shoyu Ramen - Ramen sauce soja
- Recette de ramen  Shoyu tare - Sauce pour ramen soja
tags:
- ingredient
---

# 🥕 [[katsuobushi (copeaux de bonite séchée)]]

## 📋 Informations

- **Catégorie**: 
- **Saison**: 
- **Allergènes**: 

## 🍽️ Utilisé dans les recettes

```dataview
TABLE WITHOUT ID
  file.link as "Recette",
  temps_preparation as "Préparation (min)",
  temps_cuisson as "Cuisson (min)",
  type_cuisine as "Cuisine"
FROM "contenus/recettes/Fiches"
WHERE contains(ingredients, "[[katsuobushi (copeaux de bonite séchée)]]")
SORT file.name ASC
```

## 💡 Notes


## 🔗 Liens
